...
def _setup_lock_contention_logging(base_dir: Path):
    ...
    try:
        lock_contention_log = Path(base_dir) / "lock_contention.log"
        lock_contention_log.parent.mkdir(parents=True, exist_ok=True)
...
class DistributedLock:
    ...
    def acquire(self, lock_name: str, timeout_seconds: float = 5.0) -> bool:
        ...
        """
        _setup_lock_contention_logging(self.base_dir)
