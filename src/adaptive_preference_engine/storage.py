...
from adaptive_preference_engine.models import Preference, Association, ContextStack, Signal
from adaptive_preference_engine.distributed_lock import DistributedLock
from adaptive_preference_engine.paths import get_base_dir


class JSONLStorageReadError(RuntimeError):
    """Raised when a JSONL file contains malformed records."""

    def __init__(self, filepath: Path, errors: List[Dict[str, Any]]):
        ...
        super().__init__(f"Malformed JSONL records in {filepath}: {details}{more}")
...
class JSONLStorage:
    ...
    def __init__(self, filepath: str):
        ...
        base_dir = self.filepath.parent.parent if len(self.filepath.parents) > 1 else self.filepath.parent
        self._distributed_lock = DistributedLock(str(base_dir))
        self._lock_name = f"{self.filepath.stem}_write"
        self.last_read_errors: List[Dict[str, Any]] = []

    def _acquire_write_lock(self) -> None:
        """Acquire the file's write lock or raise."""
        if not self._distributed_lock.acquire(self._lock_name):
            raise RuntimeError(f"Could not acquire write lock for {self.filepath.name}")

    def _release_write_lock(self) -> None:
        """Release the file's write lock."""
        self._distributed_lock.release(self._lock_name)
    ...
    def delete_by_id(self, obj_id: str) -> bool:
        """Delete object by ID (rewrites entire file)."""
        self._acquire_write_lock()
        try:
            data = self.read_all()
            kept = [obj for obj in data if obj.get("id") != obj_id]
...
class PreferenceStorage(JSONLStorage):
    ...
    def delete_preference(self, pref_id: str) -> bool:
        ...
        return self.delete_by_id(pref_id)
...
class AssociationStorage(JSONLStorage):
    ...
    def delete_association(self, assoc_id: str) -> bool:
        ...
        return self.delete_by_id(assoc_id)
...
class SignalStorage(JSONLStorage):
    ...
    def delete_signal(self, signal_id: str) -> bool:
        ...
        return self.delete_by_id(signal_id)
...
class PreferenceStorageManager:
    ...
    def delete_preference(self, pref_id: str) -> bool:
        """Delete a preference and remove context references to it."""
        deleted = self.preferences.delete_preference(pref_id)
        if not deleted:
            return False

        for assoc in self.associations.get_associations_for_preference(pref_id):
            self.associations.delete_association(assoc.id)

        for context in self.contexts.get_all_contexts():
            if pref_id in context.preferences:
                del context.preferences[pref_id]
    ...
    def delete_signal(self, signal_id: str) -> bool:
        """Delete a signal by ID."""
        return self.signals.delete_signal(signal_id)
    ...
    def prune_old_signals(self, max_age_days: int = 90) -> int:
        ...
        all_signals = self.signals.read_all(skip_invalid=True)
